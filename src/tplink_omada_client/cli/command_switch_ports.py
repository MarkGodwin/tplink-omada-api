...
def _print_port_table(switch: OmadaSwitch, ports: list[OmadaSwitchPortDetails]):
    ...
    for p in ports:
        print(f"{p.port:2}  {p.name[:12]:12} {p.profile_name[:10]:>10}  {get_checkbox_char(not p.is_disabled)}   ", end="")
        print(f"{get_checkbox_char(p.has_profile_override)}   {get_link_status_char(p.port_status.link_status)}  ", end="")
...
def arg_parser(subparsers) -> None:
    ...
    switch_ports_parser: ArgumentParser = subparsers.add_parser(
        "switch_ports", help="Shows detailed information about the ports on the specified switch"
    )
